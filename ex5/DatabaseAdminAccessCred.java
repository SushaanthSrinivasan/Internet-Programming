...
public class DatabaseAdminAccessCred extends HttpServlet{
	  ...
	  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String sql = "SELECT * FROM test.ADMINCRED";
			...
			while(rs.next()){
				...
				if(rs.getString("uname").equals(uname) && rs.getString("password").equals(pw)){
					...
					//rowCt+=1;
					found=1;
					response.sendRedirect("http://localhost:8080/SQLpgm/ID5");
				}
			}
			if(found==0){
			...
			out.println("<h2>Current Visits: " + visits + "</h2>");
